...
Customizable Player Models was re-audited for PR12 against physical runtime `0.6.27a` and `tom5454/CustomPlayerModels`. The release/version marker is `9dcde8fb511ed0b8be5558defb1a577ce013600b`; the Blockbench integration was audited at `9272f4f9c36a2bbd6986e6da65bf7091369cb12b`. That exact source declares plugin ID `cpm_plugin`, `min_version=5.0.0`, `variant=both`, format ID `cpm`, codec/extension `cpmproject`, while the upstream Blockbench README records Desktop Blockbench 5.1.4 as the tested editor baseline and explicitly warns that animation import may break animations. The Factory therefore keeps the authoring path beta/experimental and `HUMAN_ONLY`: `.cpmproject` remains source authority, import/export is never treated as an automated lossless round-trip, and human confirmation remains mandatory. The Toolkit and MCP never install or automatically authorize `cpm_plugin`; `mcpPolicy=NEVER` remains fail-closed even when the extension is present and version-matched.

Player Animation Library `1.1.6+mc.1.21.1` at `PlayerAnimationLibrary/PlayerAnimationLibrary@10e019f89fa25d0cd6f50fb8768586a969106911` and Player Animator `2.0.4+1.21.1` at `KosmX/minecraftPlayerAnimator@cb3227efc19ec46065597332ae265076d0f2b495` are runtime/API authorities in PR12, not Blockbench-extension authorities. The audited sources do not justify inventing a required Blockbench plugin for either profile, so both profiles keep an empty required-extension set.
