...
reference_helpers = '''function isHorizontalWhitespace(character) {\n  return character === ' ' || character === '\\t';\n}\n\nfunction hasReferenceDefinition(source) {\n  for (const line of String(source || '').split(/\\r?\\n/)) {\n    let index = 0;\n    while (index < line.length && isHorizontalWhitespace(line[index])) index += 1;\n    while (index < line.length) {\n      let markerEnd = index;\n      if (line[index] === '-' || line[index] === '+' || line[index] === '*') {\n        markerEnd = index + 1;\n      } else {\n        let digitEnd = index;\n        while (digitEnd < line.length && line[digitEnd] >= '0' && line[digitEnd] <= '9') digitEnd += 1;\n        if (digitEnd > index && (line[digitEnd] === '.' || line[digitEnd] === ')')) markerEnd = digitEnd + 1;\n      }\n      if (markerEnd === index) break;\n      let whitespaceEnd = markerEnd;\n      while (whitespaceEnd < line.length && isHorizontalWhitespace(line[whitespaceEnd])) whitespaceEnd += 1;\n      if (whitespaceEnd === markerEnd) break;\n      index = whitespaceEnd;\n    }\n    if (line[index] !== '[') continue;\n    const close = line.indexOf(']', index + 1);\n    const labelLength = close - index - 1;\n    if (close > index && labelLength >= 1 && labelLength <= 999 && line[close + 1] === ':') return true;\n  }\n  return false;\n}\n\n'''
replace_exact(
    rendered_path,
