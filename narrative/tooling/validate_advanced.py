...
def _heading_locations(lines: list[str], sections: dict[str, tuple[str, ...]]):
    aliases = {key: {alias.strip().casefold() for alias in values} for key, values in sections.items()}
...
def _section_lines(lines: list[str], locations: list[int]):
    content: list[str] = []
...
def _section_value(lines: list[str], locations: list[int]):
    content = _section_lines(lines, locations)
    return content[0].strip() if content else None


def _contract_matches(root: pathlib.Path, includes: tuple[str, ...]):
...
def _structural_issues(path, lines, contract_name, sections, missing_code, empty_code):
...
def _reference_issues(path, lines, contract_name, sections, rules, id_re, declared_ids, minimum_code, type_code, target_code):
    locations = _heading_locations(lines, sections)
    issues: list[Issue] = []
    for key, rule in rules.items():
        ...
        if len(refs) < rule.min_references:
            issues.append(Issue(minimum_code, f'{contract_name}:{key}', path, line_no))
        for ref in refs:
            if ref.split('-', 1)[0] not in rule.allowed_types:
                issues.append(Issue(type_code, f'{contract_name}:{key}:{ref}', path, line_no))
            elif ref not in declared_ids:
                issues.append(Issue(target_code, f'{contract_name}:{key}:{ref}', path, line_no))
    return issues


def _knowledge_issues(root, docs, contracts, id_re, declared_ids):
    issues: list[Issue] = []
    for name, contract in contracts.items():
        ...
        for path in _contract_matches(root, contract.include):
            lines = docs[path]
            issues.extend(_structural_issues(path, lines, name, contract.required_sections, 'missing-knowledge-required-section', 'empty-knowledge-required-section'))
            issues.extend(_reference_issues(path, lines, name, sections, contract.reference_rules, id_re, declared_ids, 'missing-knowledge-section-reference', 'invalid-knowledge-reference-type', 'missing-knowledge-reference-target'))
    return issues


def _relationship_issues(root, docs, contracts, id_re, declared_ids):
    issues: list[Issue] = []
    for name, contract in contracts.items():
        ...
        for path in _contract_matches(root, contract.include):
            lines = docs[path]
            locations = _heading_locations(lines, sections)
            issues.extend(_structural_issues(path, lines, name, contract.required_sections, 'missing-relationship-required-section', 'empty-relationship-required-section'))
            ...
            issues.extend(_reference_issues(path, lines, name, sections, contract.reference_rules, id_re, declared_ids, 'missing-relationship-section-reference', 'invalid-relationship-reference-type', 'missing-relationship-reference-target'))

            if contract.kind_section:
                found = locations.get(contract.kind_section, [])
                value = _section_value(lines, found)
                allowed = {item.casefold() for item in contract.allowed_kinds}
            ...
            if contract.autonomy_section:
                found = locations.get(contract.autonomy_section, [])
                value = _section_value(lines, found)
                allowed = {item.casefold() for item in contract.allowed_autonomy_states}
                if value is not None and value.casefold() not in allowed:
            ...
            if not contract.allow_self_relation and contract.source_actor_section and contract.target_actor_section:
                ...
                for ref in sorted(source_refs & target_refs):
...
def _relation_refs(lines, headings, id_re):
    ...
    return [
        (heading_index + 1, sorted({ref for line in _section_lines(lines, [heading_index]) for ref in id_re.findall(line)}))
        for heading_index in locations
    ]
...
def _chronology_issues(docs, declarations, contract, id_re):
    if contract is None:
        return []
    issues: list[Issue] = []
    event_ids = {record_id for record_id in declarations if record_id.split('-', 1)[0] in contract.event_types}
    chronology: set[tuple[str, str]] = set()
    causality: set[tuple[str, str]] = set()

    def add_relation(current, ref, path, line_no, edges, prefix, reverse=False):
        if ref == current:
        ...
        if ref not in event_ids:
            ...
            return
        edges.add((ref, current) if reverse else (current, ref))
    ...
    for record_id in sorted(event_ids):
        ...
        for line_no, refs in _relation_refs(lines, contract.before_headings, id_re):
            for ref in refs:
                add_relation(record_id, ref, path, line_no, chronology, 'chronology')
        for line_no, refs in _relation_refs(lines, contract.after_headings, id_re):
            for ref in refs:
                add_relation(record_id, ref, path, line_no, chronology, 'chronology', reverse=True)
        for line_no, refs in _relation_refs(lines, contract.causes_headings, id_re):
            for ref in refs:
                add_relation(record_id, ref, path, line_no, causality, 'causality')
        for line_no, refs in _relation_refs(lines, contract.caused_by_headings, id_re):
            for ref in refs:
                add_relation(record_id, ref, path, line_no, causality, 'causality', reverse=True)

    conflict_pairs: set[frozenset[str]] = set()
    for source, target in sorted(chronology):
        if (target, source) in chronology:
            pair = frozenset((source, target))
            if pair not in conflict_pairs:
                conflict_pairs.add(pair)
                path, line_no = declarations[source][0]
                issues.append(Issue('chronology-conflicting-edge', f'{source}<->{target}', path, line_no))
    cycle = _find_cycle(chronology)
    if cycle:
        path, line_no = declarations[cycle[0]][0]
        issues.append(Issue('chronology-cycle', '->'.join(cycle), path, line_no))
    if contract.causality_acyclic:
        cycle = _find_cycle(causality)
        if cycle:
            path, line_no = declarations[cycle[0]][0]
            issues.append(Issue('causality-cycle', '->'.join(cycle), path, line_no))
    return issues


def validate(root: pathlib.Path, profile: NarrativeProfile, contracts: AdvancedContracts):
...
def exit_code(issues: list[Issue]):
