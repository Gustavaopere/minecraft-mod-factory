...
def _reference_rules(value, field: str, sections: set[str], entity_types: tuple[str, ...]) -> dict[str, SectionReferenceRule]:
    ...
    for raw_key, raw_rule in value.items():
        ...
        result[raw_key] = SectionReferenceRule(allowed_types=allowed_types, min_references=min_references)
    return result
...
def _knowledge_contracts(data, entity_types: tuple[str, ...]) -> dict[str, KnowledgeEvidenceContract]:
    ...
    for raw_name, raw_contract in raw_contracts.items():
        ...
        states = _sections(
            raw_contract.get('knowledge_state_sections', {}),
            f'knowledge_evidence_contracts.{name}.knowledge_state_sections',
            allow_empty=True,
        )
        overlap = set(required) & set(states)
        if overlap:
            raise ValueError(f'knowledge_evidence_contracts.{name} logical section keys must be unique')
        rules = _reference_rules(
            raw_contract.get('reference_rules'),
            f'knowledge_evidence_contracts.{name}.reference_rules',
            set(required) | set(states),
            entity_types,
        )
...
def _relationship_contracts(data, entity_types: tuple[str, ...]) -> dict[str, RelationshipMemoryContract]:
    ...
    for raw_name, raw_contract in raw_contracts.items():
        ...
        dimensions = _sections(
            raw_contract.get('dimension_sections', {}),
            f'relationship_memory_contracts.{name}.dimension_sections',
            allow_empty=True,
        )
        ...
        rules = _reference_rules(
            raw_contract.get('reference_rules'),
            f'relationship_memory_contracts.{name}.reference_rules',
            all_sections,
            entity_types,
        )
        source = _optional_section_key(
            raw_contract.get('source_actor_section'),
            f'relationship_memory_contracts.{name}.source_actor_section',
            all_sections,
        )
        target = _optional_section_key(
            raw_contract.get('target_actor_section'),
            f'relationship_memory_contracts.{name}.target_actor_section',
            all_sections,
        )
        memory = _optional_section_key(
            raw_contract.get('memory_event_section'),
            f'relationship_memory_contracts.{name}.memory_event_section',
            all_sections,
        )
        for role_name, key in (('source_actor_section', source), ('target_actor_section', target), ('memory_event_section', memory)):
            if key is not None and (key not in rules or rules[key].min_references < 1):
                raise ValueError(
                    f'relationship_memory_contracts.{name}.{role_name} requires a reference rule with min_references >= 1'
                )

        kind_section = _optional_section_key(
            raw_contract.get('kind_section'),
            f'relationship_memory_contracts.{name}.kind_section',
            all_sections,
        )
        allowed_kinds = _strings(
            raw_contract.get('allowed_kinds', []),
            f'relationship_memory_contracts.{name}.allowed_kinds',
            allow_empty=kind_section is None,
        )
        ...
        autonomy_section = _optional_section_key(
            raw_contract.get('autonomy_section'),
            f'relationship_memory_contracts.{name}.autonomy_section',
            all_sections,
        )
        autonomy_states = _strings(
            raw_contract.get('allowed_autonomy_states', []),
            f'relationship_memory_contracts.{name}.allowed_autonomy_states',
            allow_empty=autonomy_section is None,
        )
        if autonomy_section is None and autonomy_states:
            raise ValueError(
                f'relationship_memory_contracts.{name}.allowed_autonomy_states requires autonomy_section'
            )
        ...
        contracts[name] = RelationshipMemoryContract(
            include=include,
            required_sections=required,
            dimension_sections=dimensions,
            ...
            allowed_autonomy_states=autonomy_states,
            allow_self_relation=allow_self,
        )
    return contracts
...
def _chronology_contract(data, entity_types: tuple[str, ...]) -> ChronologyContract | None:
    ...
    all_aliases = [item.casefold() for group in (before, after, causes, caused_by) for item in group]
