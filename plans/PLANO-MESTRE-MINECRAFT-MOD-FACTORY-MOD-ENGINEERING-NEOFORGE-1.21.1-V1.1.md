...
A Factory não possui pin permanente de build NeoForge. Minecraft permanece fixo em `1.21.1` e Java em `21`. No início de cada ciclo relevante de implementação ou validação, deve ser resolvida a versão estável mais recente da linha `21.1.x` compatível com Minecraft `1.21.1`; a versão exata é então pinada no build, metadata, fixtures, CI e evidências daquele ciclo para garantir reprodutibilidade. Não atualizar silenciosamente o NeoForge no meio de um gate já iniciado. A modlist física registra o ambiente efetivamente instalado, mas não congela o target de ciclos futuros. Para o ciclo I10 atual, a resolução exata é NeoForge `21.1.250`.
